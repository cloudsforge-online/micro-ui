...
export type SurfaceKey =
  | SwitcherKey
  // `foresight-admin` was here until the P13 fold. Its panel is now a section inside `admin-web`
  // (`/foresight`), it has no hostname of its own, and the key is gone rather than kept as a
  // deprecated alias: this union is what `cloudsforgeHosts()` composes an address for, so a key
  // that survives its surface is a URL this estate promises and does not serve.
  | 'hub'
  | 'signin'
  | 'site'
  | 'emberkin'
  ...
  | 'developers'
  | 'status'
  // The mining pool console. A `SurfaceKey` and deliberately not a `SwitcherKey`: it is the first
  ...
  | 'journal'
  // The public square. Not a `SwitcherKey` either, and for once that is not the accent argument:
  // its hue was already validated as the Journal's pair, so the seventh-accent objection recorded
  // on `pool` does not apply here. It stays out because the switcher is where somebody moves
  // between the things they came to DO, and a conversation is where they go afterwards. See the
  // registry row, which states the position it can be re-opened on.
  | 'agora'
  | 'explorer'
  | 'nimbus'
  | 'account'
 ...
 */
export function servesOwnBundle(s: CloudsForgeSurface): boolean {
  if (!s.servesUi) return false
  if (s.basePath === undefined) return true
 ...
 *      address on testnet.
 */
export const ENV_LABELS: ReadonlySet<string> = new Set([
  'testnet',
  'staging',
  'preview',
  'dev',
 ...
 */
export function splitEnvLabel(label: string): { subdomain: string; env: string } | null {
  if (ENV_LABELS.has(label)) return { subdomain: '', env: label }
  const cut = label.lastIndexOf('-')
  if (cut <= 0) return null
  const subdomain = label.slice(0, cut)
  const env = label.slice(cut + 1)
  if (!ENV_LABELS.has(env) || !KNOWN_SUBS.has(subdomain)) return null
 ...
 */
export function envLabel(subdomain: string, env: string): string {
  if (!env) return subdomain
