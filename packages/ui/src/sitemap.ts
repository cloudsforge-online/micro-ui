 ...
 */
import { normalisePath } from './seo.ts'
import { SURFACES, type CloudsForgeSurface } from './surfaces.ts'
 ...
 */
export const SITEMAP_SURFACES: readonly CloudsForgeSurface[] = SURFACES.filter(
  (s) => s.servesUi && s.adminOnly !== true && s.key !== 'signin',
)
 ...
 */
function partsOf(origin: string): { scheme: string; host: string } {
  const at = origin.indexOf('://')
  if (at === -1) return { scheme: 'https://', host: origin.replace(/\/+$/, '') }
...
export function sitemapUrls(
  ...
  const originOf = (s: CloudsForgeSurface): string =>
    `${scheme}${s.subdomain === '' ? '' : `${s.subdomain}.`}${host}`

  const out: SitemapUrl[] = []
  const seen = new Set<string>()
  const push = (loc: string): void => {
 ...
 */
export function sitemapXml(apexOrigin: string, extraPaths: readonly string[] = []): string {
