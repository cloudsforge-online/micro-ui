 ...
 */
export function keepNetworkInTheAddressBar(read: () => ViewedNetwork | null): () => void {
  if (typeof window === 'undefined' || !window.history) return () => undefined
  const patchable = window as PatchedWindow

  const history = window.history
  const rawReplace =
    typeof history.replaceState === 'function' ? history.replaceState.bind(history) : null
  if (rawReplace === null) return () => undefined
  ...
  const sync = (): void => {
    ...
    const viewed = read()
    if (viewed === null) {
      if (!params.has(NETWORK_QUERY_PARAM)) return
      params.delete(NETWORK_QUERY_PARAM)
    } else {
      if (params.get(NETWORK_QUERY_PARAM) === viewed) return
  ...
  if (!patchable.__cfNetworkInTheAddressBar) {
    ...
    if (typeof window.addEventListener === 'function') window.addEventListener('popstate', sync)
  }

  return sync
}

/** One patch per window, however many modules ask for it. */
type PatchedWindow = Window & { __cfNetworkInTheAddressBar?: boolean }
