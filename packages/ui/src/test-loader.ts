 ...
 */
export const REACT_SPECIFIERS: readonly string[] = [
  'react',
  'react/jsx-runtime',
 ...
 */
export function canonicalReact(options: DedupeOptions = {}): Map<string, string> {
  ...
  for (const specifier of REACT_SPECIFIERS) {
    try {
      ...
      if (specifier === 'react') {
        throw new Error(
          `@cloudsforge/ui/test-loader: no 'react' resolvable from ${from}. It deduplicates the ` +
 ...
 */
export function installReactDedupe(options: DedupeOptions = {}): Map<string, string> {
  ...
  if (typeof registerHooks !== 'function') {
    ...
    throw new Error(
      `@cloudsforge/ui/test-loader: node:module.registerHooks is unavailable (Node ${process.version}). ` +
  ...
  ;(registerHooks as (hooks: { resolve: Resolve }) => void)({
