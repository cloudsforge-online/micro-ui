 ...
 */
export function consentCookieDomains(hostname) {
...
function writeConsentCookie(decision) {
    ...
    for (const domain of consentCookieDomains(w.location.hostname)) {
        ...
        if (readCookie(CONSENT_COOKIE_NAME) === decision)
            return;
    }
}
 ...
 */
export function analyticsAllowedHere() {
    const host = globalsOf()?.location.hostname ?? '';
    if (host === '')
        return false;
    if (host === 'localhost' || host === '127.0.0.1' || host === '::1')
        return false;
    if (host.endsWith('.local') || host.endsWith('.localtest.me'))
        return false;
    return true;
}
/* ─────────────────────────────── the gate itself ─────────────────────────────── */
 ...
 */
export function trackPageView(pathname = currentPath()) {
    if (readConsent() !== 'granted' || !globalsOf())
        return;
 ...
 */
export function grantConsent(id = analyticsId()) {
    writeConsent('granted');
    const w = globalsOf();
    if (!w || id === null || !analyticsAllowedHere())
        return;
    gtag('consent', 'update', { analytics_storage: 'granted' });
    if (w.__cfAnalyticsLoaded === id)
        return;
