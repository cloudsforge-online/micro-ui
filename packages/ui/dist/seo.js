 ...
 */
import { SURFACES, servesOwnBundle, surface } from "./surfaces.js";
 ...
 */
export const COMPANY_LINE = 'Part of CloudsForge: EMBER, the chain it runs on, and every surface built on top of it.';
/**
 ...
 */
function mountAsset(s, image) {
    if (/^[a-z][a-z0-9+.-]*:/i.test(image) || image.startsWith('//'))
        return image;
    if (!servesOwnBundle(s) || !s.basePath)
        return image;
 ...
 */
export function robotsDirective(s) {
    if (!s.servesUi || s.adminOnly === true)
 ...
 */
export function surfaceMeta(key, page = {}) {
    const s = surface(key);
    const name = s.name;
    const title = page.title === undefined || page.title === name ? name : `${page.title} — ${name}`;
    return {
        title,
        ...
        path: normalisePath(`${servesOwnBundle(s) ? (s.basePath ?? '') : ''}${page.path ?? '/'}`),
...
export function metaTags(meta, origin) {
    const absolute = (p) => (origin === '' ? p : `${origin}${p}`);
    return [
...
export function canonicalHref(meta, origin) {
 ...
 * write.
 */
export function applyHead(meta, origin) {
...
export const INDEXABLE_SURFACES = SURFACES.filter((s) => robotsDirective(s) === 'index, follow, max-image-preview:large');
