...
const describe = (anchor) => typeof anchor === 'string' ? JSON.stringify(anchor) : String(anchor);
function matcher(anchor) {
    ...
    if (anchor.global || anchor.sticky) {
        ...
        throw new Error(`cite: the anchor ${String(anchor)} carries the ${anchor.global ? 'g' : 'y'} flag. ` +
 ...
 */
export function cite(file, anchor) {
    if (!existsSync(file)) {
        throw new Error(`cite: ${file} does not exist. If the repository holding it may not be checked out, use ` +
    ...
    const first = hits[0];
    if (first === undefined) {
        throw new Error(`cite: nothing in ${file} (${lines.length} lines) matches ${describe(anchor)}. ` +
            'Either the thing being cited is gone — which is the finding — or the anchor is wrong.');
    }
    if (hits.length > 1) {
        throw new Error(`cite: ${describe(anchor)} matches ${hits.length} lines of ${file} — ` +
 ...
 */
export function citeIfPresent(file, anchor) {
    if (!existsSync(file))
        return null;
 ...
 */
export function block(citation, count) {
